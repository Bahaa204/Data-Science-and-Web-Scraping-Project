...
def main() -> None:
    ...
    SUBS_IN: Path = Helpers.DATA / "streaming_services.csv"
    HOURLY_IN: Path = Helpers.DATA / "Hourly_Rate.csv"
    STEAM_IN: Path = Helpers.DATA / "steam_games.csv"
    CURRENCIES_IN: Path = Helpers.DATA / "currencies.csv"

    SUBS_OUT: Path = Helpers.CLEAN_DATA / "subscriptions_cleaned.csv"
    HOURLY_OUT: Path = Helpers.CLEAN_DATA / "hourly_wage_cleaned.csv"
    STEAM_OUT: Path = Helpers.CLEAN_DATA / "steam_games_cleaned.csv"
    CURRENCIES_OUT: Path = Helpers.CLEAN_DATA / "currencies_cleaned.csv"

    COMBINED_FILE: Path = Helpers.CLEAN_DATA / "all_clean_data_combined.xlsx"

    print("Loading subscriptions...")
    subs = pd.read_csv(SUBS_IN)

    subs["price_local"] = subs["price"].apply(Helpers.clean_price_string)
    subs["currency_code"] = subs.apply(Helpers.get_currency_code, axis=1)

    shahid = (subs["platform"] == "Shahid") & (subs["plan_type"] == "General")
    subs.loc[shahid, "price_local"] = 0.0
    subs.loc[shahid, "currency_code"] = "USD"

    subs["price_usd"] = subs.apply(Helpers.convert_to_usd, axis=1)
    ...
    subs_clean = subs_clean[
        ["platform", "region", "plan_type", "price_usd", "scraped_at"]
    ]
    ...
    steam["Current Price"] = steam.apply(
        lambda row: Helpers.clean_price_to_usd(row["Current Price"], row["Region"]), axis=1
    )
    steam["Original Price"] = steam.apply(
        lambda row: Helpers.clean_price_to_usd(row["Original Price"], row["Region"]), axis=1
    )
    ...
    if os.path.exists(CURRENCIES_IN):
        ...
        if "USD" not in df_long["currency_code"].values:
            df_long = pd.concat(
                [
                    pd.DataFrame({"currency_code": ["USD"], "exchange_rate": [1.0]}),
                    df_long,
                ],
                ignore_index=True,
            )
    ...
    steam_final = pd.DataFrame(
        {
            "steam_source": "Steam",
            "steam_type": "game",
            "steam_name": Helpers.get_col(steam, "title"),
            "steam_region": Helpers.get_col(steam, "region"),
            "steam_platform": Helpers.get_col(steam, "platform", "platforms"),
            "steam_price_usd": Helpers.get_col(steam, "current_price"),
            "steam_original_price_usd": Helpers.get_col(steam, "original_price"),
            "steam_discount_percent": Helpers.get_col(steam, "discount"),
            "steam_currency": (
                steam["currency"] if "currency" in steam.columns else "USD"
            ),
            "steam_date": Helpers.get_col(steam, "scraped_at"),
        }
    ).reset_index(drop=True)
