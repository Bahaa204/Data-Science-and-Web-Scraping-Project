...
json.dump({"Accuracy": acc_c}, open(ML_OUTPUT / "ml_rf_classifier_metrics.json", "w"))
rc = pd.DataFrame(
    {
        "Actual": yte2.values,
        "Predicted": ypc,
        "Actual_Label": pd.Series(yte2.values).map({1: "Discounted", 0: "Full Price"}),
        "Predicted_Label": pd.Series(ypc).map({1: "Discounted", 0: "Full Price"}),
    }
)
rc["Correct"] = (rc["Actual"] == rc["Predicted"]).map({True: "Correct", False: "Wrong"})
...
Xtr3, Xte3, ytr3, yte3 = train_test_split(X_po, y_wh, test_size=0.25, random_state=42)
...
Xtr4, Xte4, ytr4, yte4 = train_test_split(X_po, y_ph, test_size=0.25, random_state=42)
...
Xtr5, Xte5, ytr5, yte5 = train_test_split(X_po, y_bn, test_size=0.25, random_state=42)
...
platform_cats = df_po["Platforms"].fillna("").astype("category").cat.categories.tolist()
payoff_meta = {
