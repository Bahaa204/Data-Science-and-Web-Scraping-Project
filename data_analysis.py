...
# Loading Data using the Path lib for python
BASE: Path = Path(__file__).parent  # Parent folder of this file
CLEAN_DATA: Path = BASE / "clean_data"
OUTPUT_FOLDER: Path = BASE / "analyzed_data"
...
subs["plan_tier"] = subs["plan_type"].map(Constants.tier_map).fillna(subs["plan_type"])
...
steam_pivot_price: DataFrame = (
    steam.pivot_table(
        index="Title", columns="Region Name", values="Current Price", aggfunc="first"
    )
    .round(2)
    .reset_index()
)
...
reg_plat: DataFrame = (
    subs[subs["price"] > 0]
    .pivot_table(index="region", columns="platform", values="price", aggfunc="median")
    .round(2)
    .reset_index()
)
    ...
    for col in worksheet.columns:
        ...
        for cell in col:
            try:
                ...
                pass
        if col[0].column:
            worksheet.column_dimensions[get_column_letter(col[0].column)].width = best
...
def title_row(worksheet: Worksheet, title: str, ncols: int, bg: Constants.BLUE) -> None:
...
OUTPUT_PATH: Path = OUTPUT_FOLDER / "all_clean_data_combined_analyzed.xlsx"
workbook.save(OUTPUT_PATH)
print(f"Saved: {OUTPUT_PATH}")
print(f"Sheets ({len(workbook.worksheets)}): {[s.title for s in workbook.worksheets]}")
